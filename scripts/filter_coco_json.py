...
class CocoFilter():
    ...
    def main(self, args):
        ...
        n = args.number
        if n > 0:
            if len(self.new_images) > n:
                if args.random:
                    import random
                    random.shuffle(self.new_images)
                    random.shuffle(self.new_segmentations)
                    self.new_images = self.new_images[:n]
                    self.new_segmentations = self.new_segmentations[:n]
                else:
                    self.new_images = self.new_images[:n]
                    self.new_segmentations = self.new_segmentations[:n]
        elif args.percentage > 0:
            if len(self.new_images) > 0:
                n = int(len(self.new_images) * args.percentage)
                if n > len(self.new_images):
                    if args.random:
                        import random
                        random.shuffle(self.new_images)
                        random.shuffle(self.new_segmentations)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--number", nargs='+', dest="number", help="Max number of images in total", default=0)
    parser.add_argument("-p", "--percentage", nargs='+', dest="percentage", help="Max percentage of images in total", default=0)
    if (args.percentage > 0) and (args.number > 0):
        print("Cannot specify both number and percentage")
        sys.exit(-1)
    ...
    parser.add_argument("-r", "--random", nargs='+', dest="random", help="Randomize the chosen images", default=False)
